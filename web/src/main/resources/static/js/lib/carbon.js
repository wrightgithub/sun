var _carbonads = {
    init: function (where) {
        ...
        var placement = this.getUrlVar('placement'), serve = this.getServe(this.getUrlVar('serve'), placement), baseurl = this.getUrlVar('cd') ? this.getUrlVar('cd') : 'srv.carbonads.net';
        var forcebanner = this.getUrlVar('bsaproforce', window.location.href), ignore = this.getUrlVar('bsaprostats', window.location.href), forwardedip = this.getUrlVar('bsaforwardedip', window.location.href);
    ...
    getServe: function (serve, placement) {
        ...
        reroute['startbootstrapcom'] = 'CVYD42E';
...
function _carbonads_go(b) {
    ...
    if (typeof link[1] != 'undefined')fulllink = link[0] + '?segment=placement:' + placement + ';&encredirect=' + encodeURIComponent(link[1]); else if (link[0].search('srv.buysellads.com') > 0)fulllink = link[0] + '?segment=placement:' + placement + ';'; else fulllink = link[0];
    fulllink = fulllink.replace(/srv.buysellads.com/g, 'srv.carbonads.net');
    var el = document.createElement('span');
    el.innerHTML = '<span class="carbon-wrap"><a href="' + fulllink.replace('[timestamp]', time) + '" class="carbon-img" target="_blank"><img src="' + ad.image + '" alt="" border="0" height="100" width="130" /></a><a href="' + fulllink.replace('[timestamp]', time) + '" class="carbon-text" target="_blank">' + ad.description + '</a></span>';
    if (!_carbonads.isset(ad.removecarbon))el.innerHTML += '<a href="http://carbonads.net/" class="carbon-poweredby" target="_blank">ads via Carbon</a>';
    ...
    if (carbonjs != null)if (_carbonads.isset(_carbon_where))_carbon_where.appendChild(fdiv); else carbonjs.parentNode.insertBefore(fdiv, carbonjs.nextSibling);
...
window['_bsap_serving_callback'] = function (banner, zone, freqcap) {
    var append = function (w, data, days) {
        var c = document.cookie, i = c.indexOf(w + '='), existing = i >= 0 ? c.substring(i + w.length + 1).split(';')[0] + '%2C' : '', d = new Date();
